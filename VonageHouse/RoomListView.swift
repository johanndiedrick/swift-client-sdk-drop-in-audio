...
final class RoomModel: ObservableObject {
    ...
    func loadRooms() {
        RemoteLoader.load(urlString: API + "/rooms", body: Optional<String>.none, responseType: [RoomResponse].self) { result in
            switch result {
    ...
    func createRoom() {
        RemoteLoader.load(urlString: API + "/rooms", body: CreateRoom.Body(displayName: self.roomName), responseType: CreateRoom.Response.self) { result in
            switch result {
            case .success(let response):
